...
> Output generated by asking Claude: *"Build a government portal login page — Platforms Code compliant, KSAU-HS branding, split layout, RTL Arabic support, bilingual EN/AR"*

![Login page showcase](showcase/preview.png)

**[▶ Live demo](https://hussainaljabri.github.io/platformscode-skill/showcase/login.html)** — hosted via GitHub Pages

What this demo enforces automatically:
- Government Verification Stamp (sticky top, collapsible with 3 verification tips)
- Cookies Banner (fixed bottom, slides up, full preference panel)
- IBM Plex Sans Arabic throughout
- Saudi green `#1f7a4f` as the sole brand color via CSS tokens
- `recalcLayout()` — header top adjusts dynamically when stamp is dismissed or expanded
- Full RTL/Arabic toggle (`dir="rtl"`)
- Accessible form: `<label for>`, keyboard-navigable password toggle, `prefers-reduced-motion`
